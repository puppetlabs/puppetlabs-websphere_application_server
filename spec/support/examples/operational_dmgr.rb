...
shared_examples 'a running dmgr' do
  before(:all) do
    ...
    @ws_admin_result = on(@agent, "#{WebSphereConstants.ws_admin} -lang jython -c \"AdminConfig.getid('/ServerCluster: #{WebSphereConstants.cluster_member}/')\"", :acceptable_exit_codes => [0,1])
  end
