  ...
  before(:all) do
    ...
    @manifest = <<-MANIFEST
      websphere_application_server::ihs::instance { '#{IhsInstance.ihs_target}':
        target           => "#{WebSphereConstants.base_dir}/#{ihs_target}",
    ...
    @ports_ihs_listening.each do |port|
      result = on(@ihs_server, "ps -elf | egrep \"#{port}(\ )+1 \"", :acceptable_exit_codes => [0,1])
      @ihs_server_process.push(result.stdout) unless result.stdout.empty?
    end
  end
