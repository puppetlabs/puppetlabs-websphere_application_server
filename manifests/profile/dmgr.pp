...
define websphere_application_server::profile::dmgr (
  ...
  String $cell,
  String $node_name,
  Stdlib::AbsolutePath $profile_base  = "${instance_base}/profiles",
  ...
  Stdlib::AbsolutePath $template_path = "${instance_base}/profileTemplates/dmgr",
  Optional[String] $options           = undef,
  Boolean $manage_service             = true,
  Boolean $manage_sdk                 = false,
  Optional[String] $sdk_name          = undef,
  Boolean $collect_nodes              = true,
  Boolean $collect_web_servers        = true,
  Boolean $collect_jvm_logs           = true,
  ...
  if ! $options {
    $_options = "-create -profileName ${profile_name} -profilePath ${profile_base}/${profile_name} -templatePath ${template_path} -nodeName ${node_name} -hostName ${dmgr_host} -cellName ${cell}"
  } else {
    $_options = $options
  }
